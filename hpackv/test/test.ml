...
let decode cases =
  ...
  List.iter
    (fun (size, wire, headers) ->
      ...
      Alcotest.(check int)
        "same length" (List.length headers)
      ...
      let enc', dec' =
        Array.fold_left
          ...
          ("", decoded_headers') (Array.make 5 0)
      in
      Alcotest.(check bool)
        "encoded_again payload is smaller or equal than encoded" true
...
let rec _take_n acc i ys =
  match (i, ys) with
  | 0, _ -> acc
  | _, [] -> acc
  | _, x :: xs when i > 0 -> _take_n (x :: acc) (i - 1) xs
  | _ -> acc
