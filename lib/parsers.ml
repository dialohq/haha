...
let take_bigstring_unsafe _n =
  (* Unsafe.take n (fun bs ~off ~len -> Bigstringaf.sub bs ~off ~len) *)
  Obj.magic ()

let frame_length = lift3 parse_uint24 uint8 uint8 uint8
let frame_type = lift Frame.FrameType.parse uint8
let flags = uint8
let parse_stream_identifier n = Int32.(logand n (sub (shift_left 1l 31) 1l))
let stream_identifier = lift parse_stream_identifier BE.uint32
...
let parse_padded_payload { Frame.payload_length; flags; _ } parser =
  if Flags.test_padded flags then
    ...
    let* pad_length = uint8 in
    if pad_length >= payload_length then
      let+ () = skip (payload_length - 1) in
  ...
  | Ok _ ->
      let parse_headers length =
        lift
          ...
          (if priority then skip 5 *> take_bigstring_unsafe (length - 5)
...
let parse_error_code = lift Error_code.parse BE.uint32
...
let parse_settings_frame ({ Frame.payload_length; _ } as headers) =
  ...
  | Ok _ ->
      ...
      let+ xs = parse_settings_payload num_settings in
