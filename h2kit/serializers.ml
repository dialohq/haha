...
let write_goaway_frame ?(debug_data = Cstruct.empty) t last_stream_id error_code
    =
  let header =
    {
      Frame.flags = Flags.default_flags;
  ...
  BE.write_uint32 t (Error_code.serialize error_code);
  schedule_bigstring t ~off:debug_data.off ~len:debug_data.len debug_data.buffer
